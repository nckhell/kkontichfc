...
import { makeUrl, getCategoryFromNewsPost } from "../../utils/gamereports";
import { formatDate } from "../../utils/dateTimeFormat";

const CarouselItemReport = props => {
  const { data } = props;
  ...
  if (data.cloudinaryID) {
    imageUrl = `https://res.cloudinary.com/kkontichfc/image/upload/v1/wedstrijdverslagen/${data.cloudinaryID}`;
  } else {
    ...
    >
      <div className="gradient-wrapper flex items-center">
        <div className="px-4 container mx-auto text-left">
          <div className="news-category text-white wedstrijdverslag">
            Wedstrijdverslag
          </div>
          <a
            href={prefixURL(href)}
...
CarouselItemReport.propTypes = {
  // eslint-disable-next-line react/forbid-prop-types
  data: PropTypes.object.isRequired
};

export default CarouselItemReport;
